...
def reader():
	#number = re.compile('^\s(Number){1}\s\d*')
		...
		for line in file:
			try:
				...
				if newLine[2] == ("Populated Place" or "Civil"):
					remove_places = ['Condominium', 'Condo', 'Subdivision', 'Number', 'Addition']
					if any(x in newLine[1] for x in remove_places):
						pass
					else:
						name = newLine[1].replace(' (historical)', '')
	...
	for dir in os.listdir(path):
		...
		for line in file:
			try:
				newLine = line.split('|')
				if newLine[2] == ("Civil"):
					places = ['City of', 'Town of', 'County']
					if any(x in newLine[1] for x in places):
						name = newLine[1].replace('City of ', '').replace('Town of', '')
						long = float(newLine[10])
						lat = float(newLine[9])
						places.insert({'name':name,'class':newLine[2],'state':newLine[3],'loc': {'type':'Point','coordinates':[long, lat]}})
					else:
...
def polygon():
	...
	#print len(list)
	#print "polygon Script: ", datetime.now() - startTime
	#print len(data)
	print data
	return data
...
def check_data():
	...
	for i in polygon():
		#print i
		#print db.places.find({"name":i}).count()
		if db.places.find({"name":i}).count() > 1:
			#print "duplicate ",i
			duplicate.append(i)
		else:
			#print "original ", i
	...
	print "check_data1 Script: ", datetime.now() - startTime

def check_data2():
	startTime = datetime.now()
	duplicate = [i for i in polygon() if db.places.find({"name":i}).count() > 1]
	original = [i for i in polygon() if db.places.find({"name":i}).count() == 1]
	print "check_data2 Script: ", datetime.now() - startTime
...
def writeCSDL():
	...
	duplicates1 = [i for i in polygon() if db.places.find({"name":i}).count() > 1]
	originals1 = [i for i in polygon() if db.places.find({"name":i}).count() == 1]
	...
	csdl_file.write("\t\tand\n")
	csdl_file.write("\t\ttwitter.place.full_name cs contains_any " + '"' + state + '"' + "\n")
	csdl_file.write("\t)\n")
	...
	csdl_file.write("\t\tand\n")
	csdl_file.write("\t\ttwitter.retweeted.place.full_name cs contains_any " + '"' + state + '"' + "\n")
	csdl_file.write("\t)\n")
	...
	csdl_file.write("\t\tand\n")
	csdl_file.write("\t\ttwitter.user.location cs contains_any " + '"' + state + '"' + "\n")
	csdl_file.write("\t)\n")
	...
	csdl_file.write("\t\tand\n")
	csdl_file.write("\t\ttwitter.retweeted.user.location cs contains_any " + '"' + state + '"' + "\n")
	csdl_file.write("\t)\n")
	csdl_file.write(")\n")
